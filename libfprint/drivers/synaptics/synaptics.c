		...
		GError *error)
{
	...
	/* Assume we are either done or need to wait for interrupts.
	 * Commands that expect more data need to override this. */
	if (resp.complete)
		next_state = SYNAPTICS_CMD_NUM_STATES;
	else
		next_state = SYNAPTICS_CMD_WAIT_INTERRUPT;

	if (callback)
		callback (self, &resp, &next_state, NULL);

	if (next_state < SYNAPTICS_CMD_NUM_STATES)
			...
			void *user_data)
{
	...
	case SYNAPTICS_CMD_GET_RESP:
		...
		fpi_usb_transfer_submit (transfer, 5000, NULL, cmd_recieve_cb, user_data);
	...
	case SYNAPTICS_CMD_SEND_ASYNC:
		...
		fpi_usb_transfer_submit (transfer, 1000, NULL, fpi_ssm_usb_transfer_cb, user_data);

		break;

	case SYNAPTICS_CMD_RESTART:
		fpi_ssm_jump_to_state (ssm, SYNAPTICS_CMD_GET_RESP);
		break;
	}
}
	      ...
	      GError *error)
{
	...
	if (error) {
		callback (self, NULL, NULL, error);
	} else if (self->cmd_complete_on_removal) {
		callback (self, NULL, NULL, self->cmd_complete_error);
...
static void
cmd_submitted_cb (FpiUsbTransfer *transfer,
		  FpDevice *device,
		  gpointer user_data,
		  GError *error)
{
	...
	if (error) {
		if (callback) {
			...
			fp_warn ("Unhandled command send error: %s", error->message);
			g_error_free (error);
		}
		return;
	}
	...
	/* Start the SSM for receiving data */
		      ...
		      SynCmdMsgCallback   callback)
{
	...
	/* callback may be NULL in some cases */
	...
	fpi_usb_transfer_submit (transfer, 1000, NULL, cmd_submitted_cb, callback);
	    ...
	    GError             *error)
{
	...
	switch (resp->response_id)
	{
		...
		case BMKT_RSP_TEMPLATE_RECORDS_REPORT:
			...
			synaptics_sensor_cmd (self, self->cmd_seq_num, BMKT_CMD_GET_NEXT_QUERY_RESPONSE, NULL, 0, NULL);
			*next_state = SYNAPTICS_CMD_GET_RESP;
